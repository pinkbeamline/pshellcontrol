class PINKCLASS():
   ...
   #### CONT SCAN EXPOSURE + SPEED optimized  ###########################################################################
    def ge_SEC_EL_continous_exposure_speed(self, exposure, X0, deltaX, Xpoints, Y0, Y1, Yspeed, passes=1, sample=" "):
        ...
        Y1=float(Y1)
        ftype=4
        self.__contscan(exposure,passes,X0,deltaX,Xpoints,Y0,Y1,Ypoints,Yspeed,passes,sample,ftype)
    ...
    def __contscan(exposure,passes,X0,deltaX,Xpoints,Y0,Y1,Ypoints,Yspeed,passes,sample,ftype):
        self.__publish_fname(fname=" ")
        GE_AreaDet.stop()
        self.__ge_setup_file("ge")
        self.__ge_setup_caenels1(exposure)
